...
function OverviewTab({ 
  ...
  const maxWeekViews = Math.max(...(stats.views?.weekly || [1]))
  const unactivated = stats.users.total - stats.users.activatedUsers
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Revenue */}
        <div className="md:col-span-2 bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm font-medium text-gray-500">Total Revenue</span>
            <DollarSign className="w-4 h-4 text-green-500" />
          </div>
          <div className="text-3xl font-bold text-gray-900">€{stats.revenue.total.toFixed(0)}</div>
          <div className="flex gap-4 mt-2 text-xs">
            <span className="text-green-600 font-medium">€{stats.revenue.verified.toFixed(0)} verified</span>
            <span className="text-amber-500">€{stats.revenue.pending.toFixed(0)} pending</span>
            <span className="text-gray-400">{stats.revenue.transactionCount} transactions</span>
          </div>
          <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">
            This month: <span className="font-semibold text-gray-700">€{stats.revenue.thisMonth.toFixed(0)}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          ...
          <div className="text-xs text-gray-400 mt-1">+{stats.users.last7Days} this week · +{stats.users.last30Days} this month</div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          ...
          <div className="text-3xl font-bold text-gray-900">{stats.festivals.published}</div>
          <div className="text-xs text-gray-400 mt-1">{stats.festivals.total - stats.festivals.published} drafts · {stats.festivals.total} total</div>
        </div>
      </div>

      {/* Row 2: Views chart + Activation + Conversion */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2 bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          ...
          <div className="flex justify-between mt-2">
            <span className="text-xs text-gray-300">{stats.views.labels?.[0]}</span>
            <span className="text-xs text-gray-300">{stats.views.labels?.[stats.views.labels.length - 1]}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-gray-500">Activation Rate</span>
              <TrendingUp className="w-4 h-4 text-blue-400" />
            </div>
            <div className="text-3xl font-bold text-gray-900">{stats.users.activationRate}%</div>
            <div className="text-xs text-gray-400 mt-1">{stats.users.activatedUsers} published · {unactivated} never published</div>
            <div className="mt-2 h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${stats.users.activationRate}%` }} />
            </div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-gray-500">Free → Paid</span>
              <TrendingUp className="w-4 h-4 text-green-400" />
            </div>
            <div className="text-3xl font-bold text-gray-900">{stats.users.conversionRate}%</div>
            <div className="text-xs text-gray-400 mt-1">{stats.users.paidUsers} paid of {stats.users.total}</div>
            <div className="mt-2 h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-green-400 rounded-full transition-all" style={{ width: `${stats.users.conversionRate}%` }} />
            </div>
          </div>
        </div>
      </div>

      {/* Row 3: Plans + Founding Members */}
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">Subscription Breakdown</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            <div className="text-2xl font-bold text-gray-600">{stats.subscriptions.FREE}</div>
          ...
          <div className="text-center p-3 bg-purple-50 rounded-lg">
            ...
            <div className="text-xs text-purple-600 mt-1">Pro</div>
          </div>
          <div className="text-center p-3 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{stats.subscriptions.ENTERPRISE || 0}</div>
            <div className="text-xs text-blue-600 mt-1">Enterprise</div>
          </div>
          <div className="text-center p-3 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{stats.subscriptions.EVENT_PASS || 0}</div>
            <div className="text-xs text-green-600 mt-1">Event Pass</div>
          </div>
          <div className="text-center p-3 bg-amber-50 rounded-lg border border-amber-200">
            <div className="flex items-center justify-center gap-1">
              <Crown className="w-4 h-4 text-amber-500" />
              <span className="text-2xl font-bold text-amber-600">{stats.users.foundingMembers}</span>
            </div>
            <div className="text-xs text-amber-600 mt-1">Founding</div>
          </div>
        </div>
      </div>

      {/* Row 4: Expiring subs + Activity Feed */}
                ...
                <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-gray-800 truncate flex items-center gap-1">
                      {sub.isFoundingMember && <Crown className="w-3 h-3 text-amber-400 flex-shrink-0" />}
                      {sub.email}
                    </div>
                    <div className="text-xs text-gray-400">{sub.plan}</div>
                  </div>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ml-2 ${
                    sub.daysLeft <= 14 ? 'bg-red-100 text-red-700' :
                    sub.daysLeft <= 30 ? 'bg-amber-100 text-amber-700' :
                    'bg-gray-100 text-gray-600'
                  }`}>
                    {sub.daysLeft}d
                  </span>
              ...
              {stats.activityFeed.map((item, i) => (
                <div key={i} className="flex items-start gap-2.5 py-1.5 border-b border-gray-50 last:border-0">
                  <div className={`mt-0.5 w-2 h-2 rounded-full flex-shrink-0 ${
                    item.type === 'signup' ? 'bg-blue-400' : 'bg-green-400'
                  }`} />
                  <div className="flex-1 min-w-0">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100">
            <thead>
              <tr>
                <th className="pb-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">User</th>
                <th className="pb-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Joined</th>
                <th className="pb-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Plan</th>
                <th className="pb-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Events</th>
              </tr>
            </thead>
                ...
                <tr key={user.id} className="hover:bg-gray-50">
                  <td className="py-2.5 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900 flex items-center gap-1.5">
                      {user.isFoundingMember && <Crown className="w-3.5 h-3.5 text-amber-400" />}
                  ...
                  <td className="py-2.5 whitespace-nowrap text-sm text-gray-600">{user.festivalCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
